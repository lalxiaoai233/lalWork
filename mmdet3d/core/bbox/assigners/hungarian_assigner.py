...
class HungarianAssigner3D(BaseAssigner):
    ...
    def assign(self, bboxes, gt_bboxes, gt_labels, cls_pred, train_cfg):
        ...
        if not torch.isfinite(cost).all():
            warnings.warn(
                "HungarianAssigner3D: cost matrix contains invalid values; "
                "replacing NaN/Inf with large finite numbers to continue training."
            )
            cost = torch.nan_to_num(cost, nan=1e6, posinf=1e6, neginf=-1e6)
            cost = torch.where(
                torch.isfinite(cost), cost, torch.full_like(cost, 1e6)
            )

        # 3. do Hungarian matching on CPU using linear_sum_assignment
        cost = cost.detach().cpu().to(torch.float64)
        if not torch.isfinite(cost).all():
            warnings.warn(
                "HungarianAssigner3D: cost matrix still contains invalid values "
                "after sanitization; forcing replacement."
            )
            cost = torch.nan_to_num(cost, nan=1e6, posinf=1e6, neginf=-1e6)
            cost = torch.where(
                torch.isfinite(cost), cost, torch.full_like(cost, 1e6)
            )
        cost_np = np.nan_to_num(cost.numpy(), nan=1e6, posinf=1e6, neginf=-1e6)
        if not np.isfinite(cost_np).all():
            raise ValueError(
                "HungarianAssigner3D: cost matrix contains non-finite values even after sanitization."
            )
