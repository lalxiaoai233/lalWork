...
def create_nuscenes_infos(root_path,
    ...
    Args:
        ...
        max_radar_sweeps (int): Max number of radar sweeps. 
            Default: 10
        out_dir (str): Output directory for info files. If None, use root_path.
            Default: None
    """
    # Use out_dir if provided, otherwise use root_path
    output_dir = out_dir if out_dir is not None else root_path
    # Ensure output directory exists
    os.makedirs(output_dir, exist_ok=True)
    
    # Check if info files already exist, skip processing if they do
    test = 'test' in version
    if test:
        info_path = osp.join(output_dir, '{}_infos_test_radar.pkl'.format(info_prefix))
        if osp.exists(info_path):
            print(f'Info file already exists: {info_path}, skipping...')
            return
    else:
        info_train_path = osp.join(output_dir, '{}_infos_train.pkl'.format(info_prefix))
        info_val_path = osp.join(output_dir, '{}_infos_val.pkl'.format(info_prefix))
        if osp.exists(info_train_path) and osp.exists(info_val_path):
            print(f'Info files already exist: {info_train_path}, {info_val_path}, skipping...')
            return
    ...
    if test:
        ...
        info_val_path = osp.join(output_dir,
